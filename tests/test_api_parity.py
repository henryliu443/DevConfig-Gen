...
SAMPLE = str(EXAMPLES / "service.yaml")
...
class TestApiCliParity(unittest.TestCase):
    def _cli_output(self, output_format):
        ...
        return Path(directory.name) / f"service.{suffix}"

    def test_cli_json_matches_python_api_byte_for_byte(self):
        cli_path = self._cli_output("json")
        with tempfile.TemporaryDirectory() as directory:
            generate_from_file("service", SAMPLE, output_dir=directory, output_format="json")
            api_path = Path(directory) / "service.json"
    ...
    def test_cli_yaml_matches_python_api_byte_for_byte(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            generate_from_file("service", SAMPLE, output_dir=directory, output_format="yaml")
            api_path = Path(directory) / "service.yaml"
            self.assertEqual(cli_path.read_bytes(), api_path.read_bytes())
    ...
    def test_generate_from_file_matches_pipeline_with_loaded_context(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            from_file = generate_from_file(
                "service", SAMPLE, output_dir=directory, output_format="json"
            )
        from_pipeline = generate_pipeline(
            "service", context=context, output_format="json"
        )
        self.assertEqual(
